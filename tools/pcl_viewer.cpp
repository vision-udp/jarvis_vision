...
void printUsage(const char *progName) {
  cout << "\n\nUsage: " << progName << " [options] [scene.pcd] <model.pcl> "
                                       "[model_2.pcl] ... [model_n.pcl]\n\n"
       << "Options:\n"
       << "-------------------------------------------\n"
...
int main(int argc, char **argv) {
  ...
  // false;
  // narf_keypoint_detector.getParameloadters ().add_points_on_straight_edges =
  // true;
  ...
  while (!viewer.wasStopped()) {
    ...
    if (received_new_depth_data && depth_image_mutex.try_lock()) {
      ...
      range_image_planar.setDepthImage(depth_map, width, height, center_x,
    ...
    if (!viewer.updatePointCloud(keypoints_cloud_ptr, color_handler_keypoints,
                                 "keypoints"))
